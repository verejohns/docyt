...
module ItemValues
  RSpec.describe ItemPercentageValueCreator do
    ...
    let(:source_column) { report.columns.create!(type: Column::TYPE_ACTUAL, range: Column::RANGE_CURRENT, year: Column::YEAR_CURRENT) }
    let(:percentage_column) { report.columns.create!(type: Column::TYPE_PERCENTAGE, range: Column::RANGE_CURRENT, year: Column::YEAR_CURRENT) }
    let(:ytd_source_column) { report.columns.create!(type: Column::TYPE_ACTUAL, range: Column::RANGE_YTD, year: Column::YEAR_CURRENT) }
    ...
    let(:dependent_report_column1) { dependent_report.columns.create!(type: Column::TYPE_ACTUAL, range: Column::RANGE_CURRENT, year: Column::YEAR_CURRENT) }
    let(:dependent_report_column2) { dependent_report.columns.create!(type: Column::TYPE_ACTUAL, range: Column::RANGE_YTD, year: Column::YEAR_CURRENT) }
    ...
    describe '#call' do
      it 'creates item_value of RANGE_CURRENT for PERCENTAGE column' do
      ...
      it 'creates item_value of RANGE_YTD for PERCENTAGE column' do
        item_value = described_class.new(
          report_data: report_data,
          item: percentage_item,
