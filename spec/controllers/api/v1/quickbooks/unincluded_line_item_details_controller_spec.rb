...
module Api
  module V1
    module Quickbooks
      RSpec.describe UnincludedLineItemDetailsController do
        before do
          allow_any_instance_of(described_class).to receive(:ensure_report_access).and_return(true) # rubocop:disable RSpec/AnyInstance
          allow(::Quickbooks::UnincludedLineItemDetailsQuery).to receive(:new).and_return(unincluded_line_item_details_query)
        end
        ...
        let(:unincluded_line_item_details_query) do
          instance_double(::Quickbooks::UnincludedLineItemDetailsQuery, by_period: [line_item_detail])
        end
        let(:params) do
          {
